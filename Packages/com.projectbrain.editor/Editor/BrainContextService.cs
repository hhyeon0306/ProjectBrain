...
namespace ProjectBrain
{
    ...
    public sealed class BrainContextService
    {
        ...
        public string Read(string rootNodeId, int depth = 1, int maxNodes = 12, int maxChars = 8000)
        {
            ...
            depthOmissions.ExceptWith(visited); nodeOmissions.ExceptWith(visited); depthOmissions.ExceptWith(nodeOmissions);
            var result = new BrainContextResult
            {
                ...
                nextNodeIds = nodeOmissions.Concat(depthOmissions).OrderBy(id => id, StringComparer.Ordinal).Take(5).ToArray()
            };
