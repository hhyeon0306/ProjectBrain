...
namespace ProjectBrain
{
    public static class BrainCompletionChecks
    {
        public static int RunInEditor()
        {
            ...
            check(!status.completed && status.reasons.Any(r => r.code == "verification-unavailable") && status.reasons.Any(r => r.code == "document-unreviewed"), "Initial refusal is explicit");
            ...
            check(!service.Check().completed && service.Check().reasons.All(r => r.code == "verification-unavailable"), "Even reviewed documents cannot substitute V1");
