...
## U1-6 · 연결선 표시
U1-7에서 전체 구조도의 연결선은 직선을 기본으로 바꾸고, 역방향 관계가 있는 쌍만 완만한 곡선으로 분리한다. 둥근 끝은 유지한다. 선택 관계를 마지막에 또렷하게 그리고 노드 테두리와 간격을 둔다. 선택 관계의 화살표는 곡선의 끝 방향을 따른다. 관계 데이터/방향을 변경하거나 교차 회피를 보장하지 않는다.

U1-7 노드 위계: Project 지름22px/이름16px Bold, Domain18px/14px Bold. 이름을 우선 표시하고 클릭 범위·선 끝·선택 링은 커진 크기에 맞춘다.

## U1-8 · 선택 관계 강조
선택 노드가 보이면 직접 연결된 보이는 노드의 아이콘·이름에 강조색과 배경을 표시한다. 선택 노드는 가장 강하게, 관련 없는 노드는 읽을 수 있는 대비로 낮춘다. 관련 이름을 우선 배치하며 충돌 생략은 유지한다. 선택이 필터로 숨거나 해제되면 관련 강조/흐림을 해제한다.

## U1-10 · 검토 중심 정보 구조 (2026-09-07)
이 절은 앞의 구형 화면 설명을 대체한다. 구조도는 선택 노드별 핵심 설명과 이동, 설계 문서는 현재 구조/역할/설계 이유/등록된 의존, 작업·검증 기록은 변화/미해결/실행 근거를 관리한다. 동일 문서·기록을 복제하지 않고 코드 진입점은 범위 필터로 연결한다.
설계 문서는 읽기가 기본이며 편집은 기존 ScriptDocumentService/BrainDocumentSync를 이용한다. 읽기 모드로 이동해도 초안을 보존한다. 문서 저장·사람 확인·검증 결과·작업 완료는 별도 상태다.
작업 화면은 요약/작업/오류·미해결/테스트·검증/설정 메뉴. 작업 번호는 tasks/display-index.json에 ID와 독립적으로 저장하고 재정렬 시 재번호하지 않는다. 목록은20건 페이지, 문서 검색 목록은60건 제한/검색 안내. 오래된 결과는 실행 시점으로 표시하고 상세 진입 시 현재 snapshot과 비교한다.
오류·검증 페이지는 이미 수집한 기록을 투영한다. 별도 버그 관리 데이터 모델, 모든 런타임 오류 수집, NUnit 케이스별 상세/파일 커버리지를 구현한 것으로 표현하지 않는다. 자동 C# 의존 분석은 후속이며 등록 관계의 출처를 유지한다.
대규모 인원 사용을 지향하는 화면 분리와 점진적 펼침이다. 분산 협업 서버·동시 편집·다중 checkout 발번/병합 해결·100~200명 성능 검증은 현재 범위가 아니다.

## U1-11 · 설계 문서 탐색 보완
문서 검색과 결과 목록은 여백·구분선으로 분리한다. 뒤로가기는 최근50개 방문 위치의 문서/분류/검색/의존 탭/스크롤 위치를 보관하고 복원한다. 같은 화면 선택은 이력을 쌓지 않는다. 등록된 상위 노드가 있으면 상위 문서로 직접 이동할 수 있다. 읽기 이동은 미저장 편집 초안을 버리지 않는다. 루트와 능력 시스템 설명은 실제 구현에 맞는 스킬 사용 예시를 먼저 제공한다.
