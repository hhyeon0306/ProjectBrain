...
- A1은 기본 저장·이관 범위 완료다. 문서·기반 코드 검토에서 Git 체크아웃 줄바꿈에 따른 재이관 거절과 필수 JSON 필드 누락 수용(A1-R), 손상 문서의 UI 예외 누락(F2)을 재현했다. 기초 설계 검토는 완료했으며 추가 전면 검토 없이 구현으로 진행한다. W1/M2a 최소 계약은 확정됐으며 A1-R/F2 오류 경계는 수정·회귀 검증 완료했다. 다음은 공통 계층 서비스와 W1/M2a의 begin/context 재개 흐름을 우선한다. 세부 우선순위는 Docs/task.md를 따른다. UI 장식보다 작동하는 흐름을 우선한다.
